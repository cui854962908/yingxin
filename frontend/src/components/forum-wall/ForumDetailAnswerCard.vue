...
<template>
  <article class="fthread-ans-card" :class="{ 'fthread-ans-card--accepted': answer.is_accepted }">
    ...
    <div class="fthread-ans-inner">
      <header class="fthread-ans-head">
        <span class="fthread-avatar fthread-avatar--sm">{{ authorInitial }}</span>
        <div class="fthread-ans-meta">
          <div class="fthread-ans-name-row">
      ...
      <footer v-if="canAccept || !isGuest" class="fthread-ans-foot">
        <button
          v-if="canAccept"
          type="button"
