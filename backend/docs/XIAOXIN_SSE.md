...
与 **`POST /api/agent/chat`**（域检测 / 隐私闸 / FAQ 后再走本节同款 **`documents` + Ollama**）**共存**；本节路由为 SSE 形态的 **`POST /api/chat`**。
...
| 变量 | 说明 |
|------|------|
| `XIAOXIN_CHAT_ENABLED` | 默认 `true`；`false` 时 `POST /api/chat` **503** |
...
均写入 **`documents.embedding_json`**（Ollama HTTP 嵌入，见 **`app/core/llm.py`**）。
...
管理员在 FAQ / 公告 / 社团增删改成功后，后台会通过 **`incremental_embed_*`** 异步更新向量；若 Ollama 当时未就绪，可再跑 **`init_db.py`** 或 Python 中调用 **`rebuild_documents_best_effort()`**。

若 embedding 失败，多为 **`bge-m3` 冷启动**或与生成模型争抢资源：先 `ollama pull bge-m3:latest` 并确认 Ollama 服务可用后再重建向量。
...
**`POST /api/tts`**，body `{ "text": "..." }`，响应 **`audio/mpeg`**。
...
前端字段级说明：**`docs/FRONTEND_API.md`** §6.5–6.6。
