...
与 **`POST /api/agent/chat`**（域检测 / 隐私闸 / FAQ 后再走本节同款 **`documents` + 嵌入 API**）**共存**；本节路由为 SSE 形态的 **`POST /api/chat`**。
...
管理员在 FAQ / 公告 / 社团增删改成功后，后台会通过 **`incremental_embed_*`** 异步更新向量；若嵌入 API 当时未就绪，可再跑 **`init_db.py`**、§3 全量重建命令，或 **`rebuild_documents_best_effort()`**（详见 **`docs/ADMIN_GUIDE.md`** §4）。

小信 **`done`** 事件中的 **`links`** 在未命中知识库时可含 **牧院新生说**（`/wall`）引导。
