...
| **小信** | `POST /api/chat`（SSE） | `app/api/v1/chat.py` → **`stream_xiaoxin_events`**：**直接**走进向量 + Ollama，无前面的域检测 / FAQ 快车等。 |
...
- **Authorization**：可选。带则须合法；无效 / 过期 **401**。**个人向业务**依赖 JWT `sub`（学号），不信前端传来的 `student_id`。
...
| 小信步 | **`documents`**（由 **`faq` + `announcements`** 全量或增量写入，见 `build_documents` / `incremental_embed_*`） |
...
build_prompt(context, question) → generate_stream（Ollama /api/generate）→ 逐个 token yield
done：有 context 则 links 指向 /faq、/announcements；附带 matched_title、reply_mode（路由层收敛 done 时可只透出 links）
```
...
| `OLLAMA_*` | 嵌入 `/api/embeddings` 与生成 `/api/generate`。 |
| **`build_documents`** / **`rebuild_documents_best_effort`** | 维护 **`documents`**，源为 **`faq` + announcements**（不写 **`faqs`**）。 |
...
更细的 Agent 分支与 `source` 含义见 **`docs/AGENT_RUNTIME_FLOW.md`**。

---

**文档路径（仓库内）：** `backend/docs/AGENT_AND_XIAOXIN_DUAL_PATH.md`

**本机示例绝对路径：** `c:\Users\Lenovo\Desktop\student-welcome\backend\docs\AGENT_AND_XIAOXIN_DUAL_PATH.md`
